...
import { SEARCH_IN_CONTACT_TAGS, ADD_NEW_TAG_ORDER } from "../../../../pages/api/index";
...
export default function Tags({ TagList, setTagList, AllData }) {
  // gm : states ↓
  ...
  useEffect(() => {
    console.log("mamad " + JSON.stringify(NewTag))
    if (JSON.stringify(NewTag) !== '[]') {
      PostAuthUrl(ADD_NEW_TAG_ORDER(AllData?.id), {
        "tags": TagList
      }).then((res, err) => {
        if (res && res.status === 200) {
          if (res?.data?.isSuccess) {
    ...
    <Grid
...

      {
        ShowSearchList && focus && SearchData?.length >= 1 ? (
          <Grid style={{ marginTop: "63px", width: "92%" }} item className={styles.P_Option_Select}>
