...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditProfessionalOrganizationsCv';
...
// mrx : api links ↓
import {
    PUBLISHING_CV_PROFESSIONAL_ORGANIZATION
} from '../../../../../pages/api/index';

// mrx : api ↓
import { PostUrl, GetUrl, GetAuthUrl, PutAuthUrl } from '../../../../../pages/api/config';

export default function Items({ Data, setLocalData, LocalData, titleListIsOpen, ItemId }) {
    // mrx : context
    const {
        DataEdited,
        setDataEdited,
    ...
    const GET_ADDED_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("CreateCv-Professional-Organizations") || '[]' : '[]');
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Professional-Organizations", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
        toast.success("Your item Deleted successfully");
        setDataEdited(true);
        handleCloseRemoveCV();
        // router.push("/auth/signup/step2#ProfessionalOrganizations");
        setProfessionalOrganizationsEditData(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id));
    }

    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_PROFESSIONAL_ORGANIZATION + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Professional-Organizations", JSON.stringify(GET_ADDED_DATA &&
    ...
    return (
        ...
        >
            ...
            <Grid container justifyContent='space-between' className={`${signUpStyle.P_El}`}>
                <Grid style={{ color: Data?.isOpen ? "black" : "" }} item className={`${signUpStyle.El}`}>Organization title</Grid>
                <Grid item className={`${signUpStyle.valueEl}`}>{Data?.title ? Data?.title : "---"}</Grid>
            </Grid>

            <EditCv LocalDatae={LocalData} setLocalDatae={setLocalData} Data={Data} openModal={modalCv} handleCloseModal={() => handleCloseEditCv()} />
