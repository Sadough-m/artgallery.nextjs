...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditPublicationsCv';
...
// mrx : api links ↓
import {
    PUBLISHING_CV_PUBLICATIONS
} from '../../../../../pages/api/index';

// mrx : api ↓
import { PostUrl, GetUrl, GetAuthUrl, PutAuthUrl } from '../../../../../pages/api/config';

export default function Items({ Data, titleListIsOpen, ItemId }) {
    const router = useRouter();
    // mrx : states ↓
    const [modalCv, setModalCv] = useState(false);
    const [modalRemoveCv, setmodalRemoveCv] = useState(false);
    const [LocalData, setLocalData] = useState([]);
    const GET_ADDED_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("CreateCv-Publications") || '[]' : '[]');
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Publications", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
        toast.success("Your item Deleted successfully");
        setDataEdited(true);
        handleCloseRemoveCV();
        // router.push("/auth/signup/step2#Publications");
        setPublicationsEditData(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id));
    }

    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_PUBLICATIONS + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Publications", JSON.stringify(GET_ADDED_DATA &&
    ...
    useEffect(() => {
        localStorage.setItem("CreateCv-Publications", JSON.stringify(LocalData));
    }, [LocalData]);
    ...
    return (
        ...
        >
            ...
            <Grid container justifyContent='space-between' className={`${signUpStyle.P_El}`}>
                <Grid style={{ color: Data?.isOpen ? "black" : "" }} item className={`${signUpStyle.El}`}>Publications</Grid>
                <Grid item className={`${signUpStyle.valueEl}`}>{Data?.name ? Data?.name : "---"}</Grid>
            </Grid>
