...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditProfessionalServiceCv';
...
// mrx : api links ↓
import {
    PUBLISHING_CV_PROFESSIONALSERVICE
...
export default function Items({ CountrySelectInputData, SelectInputData, Data, titleListIsOpen, ItemId }) {
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Professional-Service", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
        toast.success("Your item Deleted successfully");
        setDataEdited(true);
        handleCloseRemoveCV();
        // router.push("/auth/signup/step2#ProfessionalService");
        setProfessionalServiceEditData(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id));
    }

    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_PROFESSIONALSERVICE + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Professional-Service", JSON.stringify(GET_ADDED_DATA &&
