...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditGrantsAndAwardsCv';
...
export default function Items({ setLocalData, LocalData, SelectInputData, Data, titleListIsOpen, ItemId }) {
    ...
    const GET_ADDED_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("CreateCv-Grants-And-Awards") || '[]' : '[]');
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Grants-And-Awards", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
    ...
    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_GRANTANDAWARDL + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Grants-And-Awards", JSON.stringify(GET_ADDED_DATA &&
