...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditExhibitionsCv';
...
export default function Items({ CountrySelectInputData, setLocalData, LocalData, SelectInputData, Data, titleListIsOpen, ItemId }) {
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Exhibitions", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
        toast.success("Your item Deleted successfully");
        setDataEdited(true);
        handleCloseRemoveCV();
        // router.push("/auth/signup/step2#SelectedExhibitions");
        setSelectedExhibitionsEditData(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id));
    }

    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_EXHIBITIONS + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Exhibitions", JSON.stringify(GET_ADDED_DATA &&
    ...
    return (
        ...
        >
            ...
            <Grid container justifyContent='space-between' className={`${signUpStyle.P_El}`}>
                ...
                <Grid item className={`${signUpStyle.valueEl}`}>{SelectInputData?.exhabitionTypes?.filter(item => item?.id === Data?.typeId).map(item => (item?.name))
                    ?
                    SelectInputData?.exhabitionTypes?.filter(item => item?.id === Data?.typeId).map(item => (item?.name)) : "---"}
                </Grid>
            </Grid>
            <Grid container justifyContent='space-between' className={`${signUpStyle.P_El}`}>
            ...
            <Grid container justifyContent='space-between' className={`${signUpStyle.P_El}`}>
                <Grid style={{ color: Data?.isOpen ? "black" : "" }} item className={`${signUpStyle.El}`}>Gallery, Instituation</Grid>
            ...
            <EditCv
                ...
                CountrySelectInputData={CountrySelectInputData} SelectInputData={SelectInputData} Data={Data} openModal={modalCv} handleCloseModal={() => handleCloseEditCv()} />
