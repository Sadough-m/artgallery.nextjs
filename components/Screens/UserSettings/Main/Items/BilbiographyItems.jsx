...
// good man : components ↓
import EditCv from '../../../../../components/Modals/CreateCV/EditBilbiographyCv';
...
export default function Items({ setLocalData, LocalData, Data, titleListIsOpen, ItemId }) {
    ...
    const GET_ADDED_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("CreateCv-Bilbiography") || '[]' : '[]');
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("CreateCv-Bilbiography", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
    ...
    const handlePublishCV = () => {
        PutAuthUrl(PUBLISHING_CV_BIOGRAPHY + `?Id=${Data?.id}&IsPublish=${!Data?.isPublished}`).then((res, err) => {
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    toast.success(res?.data?.message);
                    localStorage.setItem("CreateCv-Bilbiography", JSON.stringify(GET_ADDED_DATA &&
    ...
    useEffect(() => {
        localStorage.setItem("CreateCv-Bilbiography", JSON.stringify(LocalData))
    }, [LocalData]);
