...
export default function TableEdition({
  ...
  const handleChange = (artworkId) => {
    if (artworkId === "allSelect") {
      ...
      let tempTable = table?.map((item) =>
        item?.editionId === artworkId ? { ...item, isChecked: !item?.isChecked } : item
      );
      setCheckedItems(tempTable?.filter((item) => item?.isChecked === true)?.map((item) => item?.id))
      setTable(tempTable);
    }
            ...
            >
              <Grid item>
                <Button
                  ...
                  onClick={() => HanldeEDeleteLocalEditions()}
                >
                  Delete Editions
                </Button>
              </Grid>
          ...
          <div className={ArtWorkStyle.P_tableArtists_MobileScroll}>
            <table className={ArtWorkStyle.tableArtists_MobileScroll}>
              <tr className={ArtWorkStyle.header_table1}>
                ...
                <th className={ArtWorkStyle.headerItem}> {parseInt(AllData?.medium) === 0 ? "U" : "D"} </th>

                <th className={ArtWorkStyle.headerItem}>Price</th>
                <th className={ArtWorkStyle.headerEdit}>
                  <IconButton onClick={() => { setEditAll(false); setShowDisOrSaveLimited(true) }}>
                    <Image src={EditIcon} />
                  </IconButton>
                </th>
              </tr>
              ...
              {table?.map((item) => (
                <>
                  <tr className={ArtWorkStyle.tableData1} key={item?.editionId}>
                    ...
                    <td className={ArtWorkStyle.itemTabe}>{item?.size_Width}</td>
                    <td className={ArtWorkStyle.itemTabe}>{item?.size_Height}</td>
                    <td className={ArtWorkStyle.itemTabe}>
                      {parseInt(AllData?.medium) === 0 ? item?.sizeUnitName : item?.size_Depth}
                    </td>
                    ...
                    <td className={ArtWorkStyle.itemTabe}>
                      <IconButton
                        onClick={() => { localStorage.setItem("UploadingFileMedia", '[]'); setSignleItemId(item?.editionId); Cookies.set("Selected-item-single", item?.type); router.push(`/artwork/single`) }}
                      >
                        <Image src={EditIcon} />
                      </IconButton>{" "}
                    </td>
