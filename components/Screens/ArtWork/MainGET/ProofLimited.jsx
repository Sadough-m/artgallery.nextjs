...
// mrx : components ↓
import ProofItem from "../ProofILimitedtem";
import useWindowSize from "../../../../Hooks/useWindowSize";
import CustomSelect from "../../../Forms/CustomSelect";
import InputForm from "../../../Forms/InputForm";
import File from "../../../Forms/FileUploaderWithDrag";

export default function Proof({ AllData, SelectInputData, setProofData }) {
  // mrx : context
  const {
    ProofSectionData,
    setProofSectionData,
  ...
  useEffect(() => {
    setProofSectionData(AllData?.editions?.filter((item) => item?.id === SignleItemId)?.map((item) => item?.artworkProofs)[0] ?
      AllData?.editions?.filter((item) => item?.id === SignleItemId)?.map((item) => item?.artworkProofs)[0]?.map((item) => ({
        "title": item?.title,
      ...
      >
        ...
        <Grid item className={ArtWorkFlowStyle.titleAndBtn}>
          ...
          >
            ...
            {ProofSectionData?.length >= 1 ? ProofSectionData?.filter((item) => item?.LimitedID === SignleItemId)?.length >= 1 && (
