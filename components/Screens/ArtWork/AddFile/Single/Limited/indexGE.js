...
import DragAndDropCo from "./DragAndDrop";
...
export default function AddFile({
  ...
  const GET_Media_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("Original-Media") || '[]' : '[]');
      ...
      if (res && res.status === 200) {
        if (res?.data?.isSuccess) {
          ...
          setUploadingFileMedia(prev => [...prev, {
            ...
            "LimitedSt": LocalClassificationID === 1 ? selectButton : LocalClassificationID === 2 ? selectButton ? selectButton : "Reproduction" : "none",
          }]);
  ...
  useEffect(() => {
    if (UploadingFileMedia?.filter(item => item?.classificationType === LocalClassificationID)?.length >= 1) {
      setHaveMedia(true);
    } else {
      setHaveMedia(false);
    }
    localStorage.setItem("UploadingFileMedia", JSON.stringify(UploadingFileMedia));
    setHveData(UploadingFileMedia?.filter(item => item?.classificationType === LocalClassificationID)?.length);
    setAllMediaData(UploadingFileMedia);
  }, [UploadingFileMedia])
  ...
  useEffect(() => {
    setNumberLenght(75 * parseInt(UploadingFileMedia?.length));
  }, [UploadingFileMedia]);

  useEffect(() => {
    setNumberLenght(105 * parseInt(UploadingFileMedia?.length));
  }, [Uploading])
  ...
  const fileLength = UploadingFileMedia?.length;
                    ...
                    <Grid item>
                      <Grid
                        ...
                        style={{
                          position: "relative",
                          height: width > 960 ?
                            fileLength === 1 || fileLength === 2 || fileLength === 3 || fileLength === 4 ? "110px" :
                              Uploading && UploadingFileMedia?.length === 0 ? "100px" : handleGetHeightDesktopLimited()
                            : Uploading && UploadingFileMedia?.length === 0 ? "60px" : Uploading && UploadingFileMedia?.length === 1 ? "140px" :
                              NumberLenght === 0 ||
                                NumberLenght == 105 ? "200px" : NumberLenght == 210 ? "240px" : NumberLenght
                        }}
                      >
                        {
                          Uploading === true ?
                            <div
                              style={{
                                marginTop: width > 960 ? UploadingFileMedia?.length === 0 ? "45px" : "0px" : "50px",
...

        {
          Uploading === false &&
            ...
            <Grid item style={{ marginBottom: "-10px", height: width > 960 ? "" : "100px" }}
              {...getRootProps({ className: Uploading === false && UploadingFileMedia?.length === 0 ? "dropzoneWithNoImg" : "dropzone" })}
            >
              <input {...getInputProps()} />
