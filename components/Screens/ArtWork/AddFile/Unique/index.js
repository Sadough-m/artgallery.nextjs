...
export default function AddFile({
  ...
  // mrx : context
  const { ShowenReproduction, setShowenReproduction, HaveMedia, setHaveMedia } =
    useContext(Context);
  // states
  ...
  const GET_Media_DATA = JSON.parse(
  ...
  const handleUploadFileApi = () => {
    ...
    PostAuthUrl(
      POST_UPLOAD_FILE(localStorage.getItem("collectionId"), 1),
      File
    ).then((res, err) => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "UploadingFileMedia",
      JSON.stringify(UploadingFileMedia)
    );
    setAllMediaUnique(UploadingFileMedia);
    setHveData(
      UploadingFileMedia?.filter(
        (item) => item?.classificationType === LocalClassificationID
      )?.length
  ...
  useEffect(() => {
    setNumberLenght(UploadingFileMedia?.length > 3 ? 85 * parseInt(UploadingFileMedia?.length) : 120 * parseInt(UploadingFileMedia?.length));
  }, [Uploading])
              ...
              <Grid container direction="column" spacing={3}>
                <Grid style={{ marginBottom: "-25px" }} item>
                  <Grid
                    ...
                    {...getRootProps({ className: "dropzone1-2" })}
                  >
                    <input {...getInputProps()} />
                ...
                <Grid item>
                  <Grid
                    ...
                    style={{
                      ...
                      height:
                        width > 960
                          ? Uploading && UploadingFileMedia?.length === 0 ? "100px" : handleGetHeightDesktop()
                          : Uploading && UploadingFileMedia?.length === 0 ? "50px" : NumberLenght,
                    }}
                  >
                    {
                      Uploading === true ?
