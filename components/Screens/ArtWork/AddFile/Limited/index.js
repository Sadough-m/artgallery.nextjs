...
export default function AddFile({
  ...
  const [NumberLenght, setNumberLenght] = useState(150);
  ...
  const GET_EDITIONS2 = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("ArtWork-Editions") || '[]' : '[]');
  const ArtworkMediumType = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("Add-Artwork-MediumType") || '[]' : '[]');
  // End -----------------------------------------------------------------------------------------------------------------------

  useEffect(() => {
    if (parseInt(ArtworkMediumType) === 1) {
      setMedias(GET_EDITIONS2?.map((Item) => ({
  ...
  return (
    <>
      <section className="container">
        {RemoveEditons === false && Medias?.filter((item) => item?.editionNumber !== 0)?.length >= 1 ? (
                ...
                <Grid item>
                  <Grid
                    ...
                    style={{
                      position: "relative",
                      height: width > 960 ?
                        Uploading && UploadingFileMedia?.filter((Item) => Item?.LimitedSt == parseInt(Cookies.get("Limited-ID")))?.length === 0 ? "100px" : handleGetHeightDesktopLimited()
                        : Uploading && UploadingFileMedia?.filter((Item) => Item?.LimitedSt == parseInt(Cookies.get("Limited-ID")))?.length === 0 ? "60px" : Uploading && UploadingFileMedia?.filter((Item) => Item?.LimitedSt == parseInt(Cookies.get("Limited-ID")))?.length === 1 ? "140px" :
        ...
        {(RemoveEditons === true || Uploading === false && Medias?.filter((item) => item?.editionNumber !== 0)?.length < 1) ? (
          <Grid
            item
