...
export default function DragAndDrop({
    ...
    const [FirstItemData, setFirstItemData] = React.useState([]);
    ...
    useEffect(() => {
        setFirstItemData(Data?.filter((item) => item?.LimitedSt === Medias?.map((item) => parseInt(item?.editionNumber))[0]))
    }, [Data])
    ...
    useEffect(() => {
        if (width > 960) {
            if (sameMedia === true) {
                setBottomCheckSize("-14px");
            } else {
                if (items && items[selectButton]?.length) {
                        ...
                        Medias?.map((Item) => (
                            <div
                                style={{
                                    ...
                                    position: Item?.editionNumber === parseInt(Cookies.get("Limited-ID")) ? Item?.medias?.length >= 1 ? "" : "absolute" : "fixed",
                                }}
                                className="container"
                            >
                                <GridDropZone
