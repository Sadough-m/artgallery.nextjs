...
export default function FilterMobile({
  ...
  const ClearAllValue = () => {
    ...
    onFilter({
      category: { name: Data?.category?.filter((item) => item?.id === Statuse)?.map((item) => item?.name), id: Statuse },
      tags: { name: Data?.tags?.filter((item) => item?.id === TaggedWithData)?.map((item) => item?.name), id: TaggedWithData },
          ...
          <Grid item className={Style.BoxMenu}>
            ...
            <Grid item className={Style.P_Filters}>
              <CustomSelect
                ...
                SelectName={Data?.category?.filter((item) => item?.id === Statuse)?.map((item) => item?.name)}
              />
              <CustomSelect
                Data={Data?.tags}
                value={TaggedWithData}
                setValue={setTaggedWithData}
                setSelectName={setTaggedWithData}
                SelectName={Data?.tags?.filter((item) => item?.id === TaggedWithData)?.map((item) => item?.name)}
                label="tagged with"
              />
