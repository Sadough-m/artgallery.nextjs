...
// mrx : api links ↓
import { BASE_Image_Url, POST_UPLOAD_FILE } from "../../pages/api/index";
    ...
    PostAuthUrl(POST_UPLOAD_FILE(localStorage.getItem("collectionId"), 1),
      ...
      if (res && res.status === 200) {
        if (res?.data?.isSuccess) {
          ...
          setFileExtension(res?.data?.data?.fileExtention);

          setUploadingFileUrl(res?.data?.data?.fullPath);
          setUploadingFileUrl2(res?.data?.data?.fullPath)
  ...
  return (
    <Grid item className={`${styles.P_File}`}>
      ...
      <Grid item className={styles.posRel}>
        ...
        <input
          value={UploadingFileUrl?.length > 1 ? `Image.${FileExtension}` : ""}
          type="text"
