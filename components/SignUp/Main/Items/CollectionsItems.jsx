...
// good man : components ↓
import EditCv from '../../../../components/Modals/CV/EditCollectionsCv';
...
export default function Items({ CountrySelectInputData, setLocalData, LocalData, SelectInputData, Data, titleListIsOpen, ItemId }) {
    ...
    const GET_ADDED_DATA = JSON.parse(typeof window !== "undefined" ? localStorage.getItem("Step2-Collections") || '[]' : '[]');
    ...
    const handleDeleteItem = () => {
        ...
        localStorage.setItem("Step2-Collections", JSON.stringify(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id)));
        toast.success("Your item Deleted successfully");
        setCOlDataEdited(true);
        handleCloseRemoveCV();
        router.push("/auth/signup/step2#Collections");
        setCollectionsEditData(GET_ADDED_DATA.filter(Item => Item?.id !== Data?.id));
    }
