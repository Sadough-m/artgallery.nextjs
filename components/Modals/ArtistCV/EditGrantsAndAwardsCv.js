...
export default function EditEducationCv({ LocalDatae, setLocalDatae, openModal, handleCloseModal, Data, SelectInputData }) {
    // mrx : context
    const {
        DataEdited,
        setDataEdited,
    ...
    useEffect(() => {
        if (LocalData?.length) {
            ...
            setHaveData(false);
            localStorage.setItem("Add-Artist-Grants-And-Awards", "[]");
        }
    }, [LocalData]);
    ...
    useEffect(() => {
        localStorage.setItem("Add-Artist-Grants-And-Awards", LocalData ? JSON.stringify(LocalData) : '[]');
    }, [LocalData]);
