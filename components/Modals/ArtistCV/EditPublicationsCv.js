...
// mrx : api links ↓
import {
    VALIDATE_CV_BUILDER_PUBLICATION
...
export default function EditEducationCv({ openModal, LocalDatae, handleCloseModal, Data, SelectInputData }) {
    // mrx : context
    const {
        DataEdited,
        setDataEdited,
    ...
    useEffect(() => {
        if (LocalData?.length) {
            ...
            setHaveData(false);
            localStorage.setItem("Add-Artist-Publications", "[]");
        }
    }, [LocalData]);

    // mrx : update state for set in localstorage  ↓
    const handleSetLocalPublications = (st, data) => {
        setLocalData(data);
        if (st === true) {
            toast.success("Your item Edited successfully");
            handleCloseModal();
            setPublicationsEditData(data);
        }
    };

    // mrx : set local data for eduction ↓
    useEffect(() => {
        localStorage.setItem("Add-Artist-Publications", JSON.stringify(LocalData));
    }, [LocalData]);
        ...
        PostAuthUrl(VALIDATE_CV_BUILDER_PUBLICATION, {
            ...
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    handleSetLocalPublications(true, res?.data?.data);
                    setDataEdited(true);
                } else {
                    handleSetLocalPublications(false, res?.data?.data);
                                            ...
                                            <Grid container justifyContent='space-between' alignItems='center'>
                                                <Grid item className={ArtistStyle.P_title1}>
                                                    <Grid container alignItems='center' spacing={2} className={ArtistStyle.cv_txt}>
                                                        ...
                                                        <Grid item className={ArtistStyle.cv_profe}>Publications</Grid>
                                                    </Grid>
                                                </Grid>
                                                <Grid item>
