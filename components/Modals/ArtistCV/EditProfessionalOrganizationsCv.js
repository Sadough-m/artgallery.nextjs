...
// mrx : api links ↓
import {
    VALIDATE_CV_BUILDER_PROFESSIONAL_ORGANIZATIONLOCAL
...
export default function EditEducationCv({ LocalDatae, openModal, handleCloseModal, Data, SelectInputData }) {
    ...
    useEffect(() => {
        if (LocalData?.length) {
            ...
            setHaveData(false);
            localStorage.setItem("Add-Artist-Professional-Organizations", "[]");
        }
    }, [LocalData]);

    // mrx : update state for set in localstorage  ↓
    const handleSetLocalProfessionalOrganizations = (st, data) => {
        setLocalData(data);
        if (st === true) {
            toast.success("Your item Edited successfully");
            handleCloseModal();
            setProfessionalOrganizationsEditData(data);
        }
    };

    // mrx : set local data for eduction ↓
    useEffect(() => {
        localStorage.setItem("Add-Artist-Professional-Organizations", LocalData ? JSON.stringify(LocalData) : '[]');
    }, [LocalData]);
        ...
        PostAuthUrl(VALIDATE_CV_BUILDER_PROFESSIONAL_ORGANIZATIONLOCAL, {
            ...
            if (res && res.status === 200) {
                if (res?.data?.isSuccess) {
                    handleSetLocalProfessionalOrganizations(true, res?.data?.data);
                    setDataEdited(true);
                    // mrx : clear the items for new data
                } else {
                    handleSetLocalProfessionalOrganizations(false, res?.data?.data);
                                            ...
                                            <Grid container justifyContent='space-between' alignItems='center'>
                                                <Grid item className={ArtistStyle.P_title1}>
                                                    <Grid container alignItems='center' spacing={2} className={ArtistStyle.cv_txt}>
                                                        ...
                                                        <Grid item className={ArtistStyle.cv_profe}>Professional Organizations</Grid>
                                                    </Grid>
                                                </Grid>
                                                <Grid item>
