...
export default function EditExhibitionsCv({
    ...
    useEffect(() => {
        if (LocalData?.length) {
            ...
            setHaveData(false);
            localStorage.setItem("Step2-Professional-Service", '[]')
        }
    }, [LocalData]);
    ...
    useEffect(() => {
        localStorage.setItem("Step2-Professional-Service", JSON.stringify(LocalData));
    }, [LocalData]);
