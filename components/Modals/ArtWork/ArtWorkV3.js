...
export default function ArtWorkV1({
  ...
  // mrx : States Start ------------------------------------------------------------------------------------------------
  useEffect(() => {
    setButtonList(Data?.editions);
    setbuttonSelected(Data?.editions?.map((item) => item?.editionNumber)[0])
  }, [Data])
  //  End --------------------------------------------------------------------------------------------------------------

  useEffect(() => {
    setMedias(Data?.editions?.filter((item) => item?.editionNumber === parseInt(buttonSelected))?.map((item) => item?.artworkMedias)[0]);
  }, [buttonSelected])

  // mrx : Set Defult Media Start -------------------------------------------------------------------------------------
  useEffect(() => {
    handleChangeImage(BASE_Image_Url + Medias?.map((item) => item?.fileUrl)[0])
  }, [Medias])
  //  End --------------------------------------------------------------------------------------------------------------
                    ...
                    <Grid item className={ArtWorkFlowStyle.P_buttonsTop}>
                      <Grid container spacing={3}>
                        <Grid item>
                          <Button
                            variant={
                              buttonSelected === "Limited edition"
                                ? "contained"
                                : ""
                            }
                            color={
                        ...
                        </Grid>
                        {
                          ButtonList && ButtonList?.map((item) => (
                            <Grid item>
                              <Button
                                variant={
                                  parseInt(buttonSelected) === item?.editionNumber ? "contained" : ""
                                }
                                color={parseInt(buttonSelected) === item?.editionNumber ? "secondary" : ""}
                                className={
                                  parseInt(buttonSelected) !== item?.editionNumber
                                    ? ArtWorkFlowStyle.btnBgwhite
                                    : ArtWorkFlowStyle.btnSelectedTop
                                }
                                onClick={() => setbuttonSelected(item?.editionNumber)}
                              >
                                {item?.editionNumber}
                              </Button>
                            </Grid>
