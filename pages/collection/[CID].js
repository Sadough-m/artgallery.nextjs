...
export default function Add() {
  ...
  useEffect(() => {
    setMediumType(Collection[0]?.mediumType);
    setStatus(Collection[0]?.status);
    setCollectionType(Collection[0]?.type);
    setCollectionName(Collection[0]?.name);
    setUploadingFileUrl(Collection[0]?.pictureUrl);
    setfileExtension(Collection[0]?.fileExtension)
  }, [Collection])
    ...
    GetAuthUrl(TEST_COLLECTIONS).then((res, err) => {
      if (res && res.status === 200) {
        if (res?.data?.isSuccess) {
          setCollection(res.data?.data?.filter((item) => item?.id === CID));
        } else {
  ...
  const handleEditNewCollection = () => {
    if (
      ...
      PostAuthUrl(EDIT_COLLECTION(collectionID), {
        "type": collectionType,
          ...
          <Grid item className={ColStyle.headerMobile}>
            <Grid container justifyContent="space-between" alignItems="center">
              ...
              <Grid item>
                <Button
                  className="Br8"
                  onClick={() => handleEditNewCollection()}
        ...
        <Grid item className={ColStyle.middle_wrapper}>
          ...
          <Grid item className="mt_20">
            <CustomSelect
              ...
              SelectName={createCollectionData?.type?.filter((item) => item?.id === collectionType)?.map((item) => item?.name)}
              setSelectName={setCollectionType}
          ...
          <Grid item className="mt_20">
            <CustomSelect
              ...
              SelectName={createCollectionData?.mediumType?.filter((item) => item?.id === mediumType)?.map((item) => item?.name)}
